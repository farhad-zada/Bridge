...
import {OwnableUpgradeable} from "@openzeppelin/contracts-upgradeable-4.7.3/access/OwnableUpgradeable.sol";
import {Initializable} from "@openzeppelin/contracts-upgradeable-4.7.3/proxy/utils/Initializable.sol";
import {UUPSUpgradeable} from "@openzeppelin/contracts-upgradeable-4.7.3/proxy/utils/UUPSUpgradeable.sol";
...
contract Bridge is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /// @notice The initialize function is used to set the initial values of the contract.
    /// @dev This function is called only once during the initial deployment of the contract.
    ...
    /// @notice The upgrade function is used to upgrade the contract to a new address.
    /// @dev This function is called only once during the initial deployment of the contract.
        ...
        returns (bytes32 messageId)
    {
        if (_token.allowance(msg.sender, address(this)) < _amount) {
            revert("Bridge: transferTokensPayLINK: Insufficient allowance");
        }
        ...
        _token.approve(address(s_router), _amount);
    ...
    {
        if (_token.allowance(msg.sender, address(this)) < _amount) {
            revert("Bridge: transferTokensPayNative: Insufficient allowance");
        }
